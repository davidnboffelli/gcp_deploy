...
resource "google_service_account_iam_member" "gce-default-account-iam" {
  ...
  service_account_id = "${google_project.doer_projects["${each.value.name}-service"].id}/serviceAccounts/${google_project.doer_projects["${each.value.name}-service"].number}-compute@developer.gserviceaccount.com"
